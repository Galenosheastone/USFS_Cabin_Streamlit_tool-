...
- Leave sender credentials out of `config.yaml`. For Streamlit, store the sender
  account privately in secrets instead (see [setup_gmail.md](setup_gmail.md)).
...
To enable email sending in Streamlit without exposing credentials publicly, add
private secrets:
...
On Streamlit Community Cloud, add those values in the app's **Secrets** settings.
For local development, put them in `.streamlit/secrets.toml` and keep that file out
of Git.

For the CLI, you can also supply private sender settings with environment variables:

```bash
export FS_CABIN_SENDER_EMAIL="your-gmail@example.com"
export FS_CABIN_SENDER_PASSWORD="your 16-character app password"
export FS_CABIN_RECIPIENTS="you@example.com"
