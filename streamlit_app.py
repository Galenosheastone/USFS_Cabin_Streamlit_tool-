...
def recipients_to_text(recipients: list[str]) -> str:
    return "\n".join(recipients)
...
def parse_recipients_text(raw_text: str) -> list[str]:
    recipients = [line.strip() for line in raw_text.splitlines() if line.strip()]
    if not recipients:
        raise ValueError("Add at least one recipient email before saving.")
    return recipients
...
def render_quick_start(current_campground_count: int) -> None:
    with st.expander("How to use this app", expanded=True):
        st.markdown(
            ...
            "4. Optional: enter your email in the sidebar if you want alerts sent to a session-specific address.\n"
            "5. Go to **Run Checker** and click **Run availability check**.\n"
            "6. Open **Email Preview** to review the alert and send it."
        )
        st.caption(
...
runtime_recipient_input = st.sidebar.text_input(
    ...
    help="Optional. If filled in, manual sends from this app session go to this address instead of the saved config recipients.",
)
...
with run_tab:
    ...
    if run_clicked:
        ...
        try:
            ...
            st.success("Availability check finished and state was saved.")
        except Exception as exc:
...
with settings_tab:
    ...
    with st.form("settings_form"):
        ...
        email_1, email_2 = st.columns(2)
        sender_email = email_1.text_input(
            "Sender email",
            value=config["email"].get("sender_email", ""),
        )
        sender_password = email_2.text_input(
            "Gmail app password",
            value=config["email"].get("sender_password", ""),
            type="password",
        )

        smtp_1, smtp_2 = st.columns(2)
        smtp_server = smtp_1.text_input(
            "SMTP server",
            value=config["email"].get("smtp_server", "smtp.gmail.com"),
        )
        smtp_port = smtp_2.number_input(
            "SMTP port",
            min_value=1,
            step=1,
            value=int(config["email"].get("smtp_port", 587)),
        )

        recipients_text = st.text_area(
            "Recipients (one email per line)",
            value=recipients_to_text(config["email"].get("recipients", [])),
            height=120,
        )
    ...
    if save_clicked:
        try:
            ...
            updated_email["smtp_server"] = smtp_server.strip()
            updated_email["smtp_port"] = int(smtp_port)
            updated_email["sender_email"] = sender_email.strip()
            updated_email["sender_password"] = sender_password
            updated_email["recipients"] = parse_recipients_text(recipients_text)
...
with email_tab:
    ...
    if not preview:
        ...
        runtime_recipient_error = None
        try:
            runtime_recipient = parse_runtime_recipient(st.session_state.get("runtime_recipient", ""))
        except ValueError as exc:
            runtime_recipient_error = str(exc)
            st.error(runtime_recipient_error)
            runtime_recipient = None

        effective_recipients = (
            [runtime_recipient]
            if runtime_recipient
            else st.session_state["config_data"]["email"].get("recipients", [])
        )
        ...
        if not st.session_state["config_data"]["email"].get("enabled", True):
            ...
            if send_now:
                if runtime_recipient_error:
                    ...
                    send_now = False
            if send_now:
                send_config = build_send_config(st.session_state["config_data"], runtime_recipient)
                if send_email(
                    send_config,
                    preview["subject"],
                    preview["html_body"],
                    ...
                    st.error("Email send failed. Check the server logs for details.")
